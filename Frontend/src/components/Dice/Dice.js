...
const DiceRoll3D = () => {
  ...
  const rollDice = async () => {
    ...
    const result = await fetchRandomDiceNumber();
  ...
  const fetchRandomDiceNumber = () => {
    return new Promise((resolve) => {
      // Simulate a network request with a random delay (2-5 seconds)
  ...
  const faceRotation = {
    ...
    2: { rotateX: 0, rotateY: -90 },      // Left
    3: { rotateX: 90, rotateY: 0 },       // Top
    4: { rotateX: -90, rotateY: 0 },      // Bottom
    5: { rotateX: 0, rotateY: 90 },       // Right
